...
def _get_previous_inniscarra_flow(s3_bucket: str) -> Optional[float]:
    ...
    Returns the flow from the last uploaded reading (`latest_reading.flow_rate_m3s`).
    This is always the true previous value — if the ESB Hydro PDF is stale, the
    current and previous readings will be identical, producing a delta of zero and
    correctly suppressing the alert.
    """
    import boto3
